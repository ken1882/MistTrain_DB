...
def patch_scenes(sids):
  ...
  for sid in sids:
    ...
    src = f"{_G.DCTmpFolder}/jh_scenes/{sid}.json"
    dst = f"{_G.STATIC_FILE_DIRECTORY}/jh_scenes/{sid}.json"
...
def save_meta(meta, prefix=''):
  for k, fname in _G.SCENE_METAS.items():
    path = f"{_G.STATIC_FILE_DIRECTORY}/jh_json/{prefix}{fname}"
...
def save_sponsors(token):
  ...
  dat = []
  filename = f"{_G.STATIC_FILE_DIRECTORY}/jh_sponsors.json"
